...
`/v2/coach/*`(2개)와 **`POST /v2/reports`**는 Gemini를 호출하므로(`coaching.py:170`, `reports.py:126`) M3에서 구현할 수 없다. M3에서는 **저장 계층만** 만들고 엔드포인트 노출은 M4가 한다.
...
| # | 그룹 | 주된 위험 |
|---|---|---|
| 1 | `/v2/me`, `/v2/consents` | `DISTINCT ON` 2건(`store.py:419`, `478`), nickname 정규화 |
...
| 6 | `/v2/admissions`, `/admin` | 조건부 LEFT JOIN, `admin_sessions`의 N+1(`store.py:2089`), admin은 조건부 등록 |
...
`community_anonymous_aliases`에는 unique 제약이 **둘** 있다(`models.py:663-668`):
...
원본은 SAVEPOINT + 3회 재시도였으나(`community_store.py:501-543`) `JpaTransactionManager`가 `PROPAGATION_NESTED`를 지원하지 않고 Hibernate가 제약 위반 후 세션 재사용을 보장하지 않으므로, **이식이 아니라 재작성이 유일한 답이다.** 관찰 가능한 동작(같은 사용자는 같은 번호, post 안에서만 유효, 번호는 1부터 연속)은 동일하게 유지한다.
...
| 항목 | 실제 동작 | 근거 |
|---|---|---|
| **차단 필터** | `anonymous = true OR author_id NOT IN blocked` — **익명 글은 숨기지 않는다.** 숨기면 차단 전후 비교로 익명 신원을 추론할 수 있다 | `community_store.py:149-156` |
| **조회수** | 상세 조회 응답은 **증가 전** `view_count`를 반환한다. 증가는 별도 원자 연산 | `community.py:287-294` |
| **입력 정규화** | title·body는 trim. **nickname은 내부 공백까지 접는다** | `community.py:121`, `profile.py:40` |
...
- [ ] 동시성: 세션 생성 경합, 재분석 경합, lease 경합 (`test_db_store.py:422`, `481`, `751` 대응)
