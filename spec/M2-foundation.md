...
- `CoachSession.id`가 그 예외 — `server_default`가 없고 agent 모듈의 문자열 파싱에서 온다(`db/store.py:PostgresStore._add_coach_session`)
...
**회전 규칙**: 소진된 refresh 토큰을 재사용하면 **해당 유저의 전 세션을 무효화**한다(`db/store.py:PostgresStore.issue_refresh_token`). 의도된 동작이며 `/SPEC.md` §6 #6이다.

평문 JWT를 DB에 저장하지 않는다 — `token_hash`는 SHA-256 64자 hex.

**양방향 상호운용이 롤백 안전성을 결정한다.** M5에서 롤백하면 사용자는 **Java가 발급한 access·refresh 토큰을 들고 FastAPI로 돌아온다**(`auth/router.py:build_router.refresh`). Python이 그 헤더·클레임·DB hash를 받아들이지 못하면 "초 단위 복구" 직후 전 사용자가 로그아웃된다. 따라서 **Python→Java와 Java→Python을 둘 다 검증**한다.
...
- **development** (`auth/development.py`) — `DEVELOPMENT_AUTH_PROVIDER=1`일 때만 등록. **기본값이 '열림'이 되지 않게** 한다(`app.py:create_app`)

`ProviderRegistry`(`auth/providers.py`)가 `unsupported_provider` 400을 낸다.

### F. 레이트리밋

`ratelimit.py` — **사용자당 60회/분, 인메모리 고정 윈도우**. `clock` 주입 가능해야 테스트가 된다(`app.py:create_app`).

초과 시 429 `rate limit exceeded`(공백 포함 문장).

### G. consent 게이트

`auth/dependencies.py:build_consent_gate_dependency`. 필수 약관 미동의 시 403 `consent_required`.
...
**프로필(`/v2/me`)은 동의 게이트를 걸지 않는다** — 동의 화면에서 이름을 함께 받기 때문(`app.py:create_app`).
...
- 코드가 `datetime.now(timezone.utc)` 생성 후 전달 — `db/store.py:PostgresStore.issue_refresh_token` 외 20여 곳
- `func.now()` 대입 — `db/store.py:PostgresStore.update_user_nickname`, `db/community_store.py:CommunityStore.update_post` 계열

**리스 만료 비교(`db/store.py:PostgresStore.claim_external_operation`·`.claim_next_external_operation`·`.sweep_max_attempts_operations`)가 앱 시계와 DB 저장값을 비교**하므로 클럭 스큐가 버그가 된다. 한쪽으로 명시하고 그 이유를 적는다.

주의: `updated_at`에 `func.now()`를 쓰고 `created_at`은 `server_default`인 테이블에서, 앱 시계로 바꾸면 한 행 안에서 `updated_at < created_at`이 나올 수 있다.

### 결정 2 — `SKIP LOCKED`

현재 0건이다. `claim_next_external_operation`(`db/store.py:PostgresStore.claim_next_external_operation`)은 후보 1건을 서브쿼리로 고르고 UPDATE의 WHERE에 조건을 다시 적용한다. 워커 2개가 경합하면 하나가 블로킹 대기 → 락 해제 후 조건 재평가 실패 → 0행 → `None` → 폴링 재시도.

**기본 방침은 원본 동작 재현이다**(`/SPEC.md` §7-4). 개선하려면 워커 폴링 로직(`analysis_worker.py:AnalysisWorker.run_once`)과 동시성 테스트(`tests/test_db_store.py:test_external_operation_idempotency_lease_race_and_atomic_completion`)를 함께 검토하고 근거를 남긴다.
