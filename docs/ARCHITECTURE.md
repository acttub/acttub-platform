...
# Abandoned upload cleanup and quota (migration 023)

Upload retention is owned by a separate `worker:upload-cleanup` process, not by browser lifecycle events, request handlers, or the analysis worker. PostgreSQL admits upload intents under a per-user transaction advisory lock and counts active intent rows plus `coalesce(actual_size_bytes, expected_size_bytes)`. Trusted Storage observation persists actual bytes and fences over-limit or mismatched media before consumption.

Cleanup is a durable lease/CAS saga in `upload_cleanup_jobs`. The worker claims bounded jobs with `FOR UPDATE SKIP LOCKED`, deletes exactly one canonical `practice-videos` path through the service role, treats an already-absent object as success, and releases quota only when live-lease completion records `cleanup_completed_at`. Session/take references and queued or active analysis remain deletion guards.
