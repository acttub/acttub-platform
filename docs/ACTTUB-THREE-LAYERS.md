...
모델은 `lookup` 또는 `respond`만 반환한다. `lookup`은 이 세션의 저장된 텍스트만 읽으며 S3·원본 영상·다른 세션에 접근할 수 없다. 대사·구간·검색어로 앞뒤 근거를 조회하고 결과를 다음 생성 입력에 실제로 넣는다. 한 번의 사용자 요청에서 조회 최대 2회, 전체 생성 최대 4회다. 조회는 화면의 대화 턴에 포함하지 않는다.
...
서버가 검증된 state/revision으로 `acttub.coach_handoff.v1`을 만들고, `acttub.practice_note.v1`을 조립한다. 모델은 제목과 선택적 짧은 정리만 작성한다. 정리 문장은 이미 기록된 문장과 출처를 그대로 골라야 한다. 생성 실패 시에도 기본 제목으로 노트를 저장한다.
...
공개 응답은 `acttub.public_practice_note.v1`, `report_type=practice_note`다. 제목·방향·구간·연습·비교 기준·실행 보고·열어 둔 부분을 표시하며 내부 상태와 과거 메시지 원문 카탈로그는 제외한다. 웹과 앱은 새 타입과 기존 analysis/expression 노트를 각각 렌더링한다. 모르는 타입을 expression으로 취급하지 않는다.
