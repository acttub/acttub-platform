...
2026-09-06 Cloudflare DNS 실조회에서 apex와 `www`는 모두 CNAME
`acttub-fe-alb-426661242.ap-northeast-2.elb.amazonaws.com`, 프록싱 켜짐·TTL 자동이었다.
현재 운영 도메인은 ALB를 직접 가리키며, 별도로 남은 CloudFront 배포와 구분한다. DNS 원복 값은
문서의 예전 구조 그림이 아니라 이 실조회 기록을 사용하고 실행 전에 다시 대조한다.
...
홈서버 유입·워커를 계속 막고, 소스 RDS가 최종 덤프 이후 변경되지 않았음을 확인한다. 이전 DNS·AWS 원본
...
4. AWS 앱을 검증한 DB로 기동하고 DNS·AWS 원본 경로를 이전 값으로 복원한다. health와 로그인·업로드·분석을
