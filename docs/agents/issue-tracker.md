...
- **보드에 뜨게 만듭니다.** 백로그에만 있으면 아무도 못 봅니다 — 만든 이슈는 두 필드를 채웁니다.
  - **스프린트** (`customfield_10020`) — 활성 스프린트 id. 조회는 기존 이슈 하나의 이 필드를 읽어
    `state: "active"`인 항목을 찾으면 됩니다(스프린트는 주마다 바뀌므로 값을 여기 적어두지 않습니다).
  - **에픽** (`parent`) — 일감이 속한 에픽. 백엔드·인프라는 `SOMA-371`입니다. 어디 붙일지 애매하면
    비슷한 기존 이슈의 `parent`를 따릅니다.
- **담당자는 비워둡니다.** 누가 잡을지는 사람이 정합니다.
