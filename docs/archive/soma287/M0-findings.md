...
### ✅ 관문 ③ production-envelope — **2026-08-12 통과, SDK 채택 확정**

적대적 리뷰가 "6초·80KB 1건은 대표성이 없다"며 채택을 **잠정 결정**으로 낮췄던 것을 여기서 닫는다
(`ProductionEnvelopeSpikeTest`, 4 tests / 0 failed). 실행은 `./gradlew test -PenvelopeSpike` 로만 켠다.

**봉투의 실측치** — 영상은 `prepareEnvelopeVideos` 가 합성한다(저장소에 커밋하지 않음):

| 값 | 실측 |
|---|---|
| 원본 (상한 100MB 근접) | **98,414,903 B** |
| 같은 영상의 압축본 (`compress.py` 와 같은 파라미터) | **2,445,640 B** — **압축률 2.5%** |
| 98MB 원본 업로드 → `ACTIVE` | **23.5초** |

| 검증 항목 | 결과 |
|---|---|
| post-compression 업로드 → 폴링 → 구조화 출력 → 엄격 파싱 | ✅ `ObservationPack` 형상으로 파싱됨 |
| **압축 실패 경로: 98MB 원본 업로드** | ✅ **다중 chunk 가 열리고 23.5초에 ACTIVE.** 최대 위험이었다 |
| `files.delete` 예외 주입 | ✅ 이미 지운 파일 재삭제가 실제로 예외를 던지고, 삼키면 분석 결과가 남는다 |
| 파싱 재시도 (`[실패,성공]` / `[실패,실패]`) | ✅ **총 2회 시도**, 세 번째를 보지 않는다 (`ParseRetryLoop`) |

🔎 **압축률 2.5%가 이 스파이크의 가장 쓸모 있는 부수 사실이다.** 운영에서 Gemini 로 가는 것은 보통
2~3MB 이고, 98MB 가 그대로 가는 것은 압축이 실패했을 때뿐이다 — 즉 **다중 chunk 경로는 예외 경로다.**
다만 그 예외 경로가 열린다는 것을 확인했으므로 `compress_for_gemini` 의 폴백 5경로를 그대로 옮겨도
업로드 단계에서 막히지 않는다.

⚠ **합성 영상이라 분석 품질은 보지 않았다.** `testsrc` 패턴에는 관찰할 연기가 없어 `observations=1` 이
나왔다. 이 스파이크가 답한 것은 전송·실패 경로이고, 프롬프트 정확도는 §A 의 요청 golden 이 판정한다.

⏳ **관문 ③의 다섯째 항목(워커 lease 3회 예산 소진)은 여기서 돌리지 못했다.** Java 에
`sweep_max_attempts_operations` 대응물이 아직 없다 — **§F-1 의 산출물**이라 그 사이클에서 확인한다.
사양이 이 항목을 관문 ③에 넣은 것은 "§A 착수 전"이라는 뜻이었는데, 실제로는 §F-1 뒤라야 성립한다.
...
| **[medium]** Gemini PASS(6초·80KB 1건)는 대용량·재시도·delete 실패를 대표하지 못한다 | ~~**M4 관문 ③**~~ → **2026-08-12 해소.** 98MB 원본 업로드까지 통과해 **SDK 채택 확정**(위 C 절). lease 소진 항목만 §F-1 로 이월 |
