...
`apps/web`과 `apps/api`를 훑어 나온 후보를 실행 단위로 쪼갠 문서다. **실행 네 단계가 전부 끝나 2026-08-24 여기로 옮겼다.**

> 📦 **보관 문서 — 실행 단계는 전부 끝났다.** 아래 서술은 리뷰 시점의 기록이고, 지금 재면 틀리는 것이 섞여 있다.
> - **1단계 — 완료.** 살아있는 경로가 `buildPracticeSessionRequest`를 호출한다(`features/workspace/practice-start.ts:185`). 아래가 호출부로 지목한 `workspace-app.tsx`는 그 뒤 `SOMA-409`가 갈랐다.
> - **2단계 — 완료(`24ac0b1`).** 삭제 대상 3파일과 그 소스 텍스트를 단언하던 테스트 2개가 사라졌다. 열린 결정 ①은 권고대로 (b)로 갔다 — `analysisFailure`는 지금 저장소에 없다.
> - **3단계 — 완료(`cc416b9`) · 4단계 — 완료(`9d5ad73`).** 둘 다 2026-08-08에 아래가 제안한 커밋 메시지 그대로 실행됐다. 이 문서가 한때 "3·4단계는 대상이 사라졌다"고 적은 것은 **틀렸다** — `SOMA-403` 5단계가 지운 것은 이미 정리된 뒤의 파이썬 트리다.
> - **ADR 개정 두 건 — 둘 다 완료.** `ADR-003`·`ADR-AI-006` 모두 2026-08-08 개정 블록이 붙었다(후자는 3단계와 같은 PR에서). `SOMA-443` 티켓 03이 `ADR-AI-006`에 덧붙일 것은 **다른 사안**이다(FastAPI·인메모리 세션·JSON 이력 전제).
>
> ⚠️ **아래 「이관이 끝난 뒤로 미룬 것」·「이번에 손대지 않는 큰 후보」는 실행 단계가 아니라 관찰이고, 그중 넷은 지금도 참이다** (2026-08-24 실측): `wait()`·`abortReason()`이 `v2/idempotency.ts:37,41`과 `v2/sessions.ts:37,41`에 두 벌 · abort 메시지가 `media/abort.ts`와 `v2/uploads.ts:65`에 다른 문구로 두 벌 · `AuthUser`가 `token-store.ts:1`(수기)과 `v2/types.ts:16`(생성)에 두 벌 · 소스 텍스트를 단언하는 `assert.match`는 139에서 **161로 늘었다.**
