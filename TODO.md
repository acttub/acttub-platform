...
- [ ] `GET /v2/practice-sessions/{id}/status` 폴링 경량 엔드포인트 신설 — 분석 대기 폴링이 상세 응답·presign 발급 없이 `{status, error_code}`만 받도록
- [ ] `GET /v2/practice-sessions?filter=incomplete` 미완료 연습 조회로 개편 — 리포트 없는 세션(created/analyzing/analyzed/failed, hidden 제외)만 슬림 응답으로; 이전 기록 화면의 진행 중 표시를 이것으로 교체
