...
def actor_memory(ctx) -> None:
    """코치가 배우를 기억하는 6칸 — 조회·수정·삭제.

    에이전트가 채우고 배우가 고치는 구조라, 이 화면이 잘못 적힌 기억을 되돌리는
    유일한 경로다. 빈 칸은 값이 없는 게 아니라 항목 자체가 빠져서 온다.
    """
    headers = ctx.auth(USER1)

    ctx.call("memory-empty", "get", "/v2/me/memory", headers=headers)

    # 배우가 직접 쓴다. 성별·나이는 에이전트가 못 쓰는 칸이다.
    ctx.call(
        "memory-put-gender",
        "put",
        "/v2/me/memory/{field}",
        path_params={"field": "gender"},
        json={"value": "여성"},
        headers=headers,
    )
    # 앞뒤 공백은 잘리고 가운데 연속 공백은 하나로 접힌다.
    ctx.call(
        "memory-put-goal",
        "put",
        "/v2/me/memory/{field}",
        path_params={"field": "goal"},
        json={"value": "  입시   준비  "},
        headers=headers,
    )
    ctx.call("memory-after-put", "get", "/v2/me/memory", headers=headers)

    # 공백만 있는 값은 빈 칸을 만드는 것이므로 거절한다.
    ctx.call(
        "memory-put-blank",
        "put",
        "/v2/me/memory/{field}",
    ...
    # 6칸 밖의 이름은 경로에서 걸린다.
    ctx.call(
        "memory-put-unknown-field",
        "put",
        "/v2/me/memory/{field}",
        path_params={"field": "height"},
        json={"value": "170"},
        headers=headers,
    )

    # 지우기가 되돌리기 수단이다. 이미 없는 칸을 지워도 결과가 같으므로 204 다.
    ctx.call(
        "memory-delete-field",
        "delete",
        "/v2/me/memory/{field}",
    ...
    ctx.call(
        "memory-delete-field-again",
        "delete",
        "/v2/me/memory/{field}",
        path_params={"field": "goal"},
        headers=headers,
    )
    ctx.call("memory-delete-all", "delete", "/v2/me/memory", headers=headers)
    ctx.call("memory-after-delete", "get", "/v2/me/memory", headers=headers)
