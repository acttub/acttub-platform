 ...
 * apps/web 의 supertonic/helper.js 를 네이티브(onnxruntime-react-native)로 포팅.
 * 원본: supertone-inc/supertonic web/helper.js (MIT)
 *
 * 웹 대비 바뀐 것:
 *  - import: onnxruntime-web → onnxruntime-react-native
 *  - fetch 기반 로더(loadCfgs/loadTextProcessor/loadVoiceStyle) 제거 → 파일시스템에서 읽은
 *    JSON 객체를 직접 넘겨 받는다(loadVoiceStyleFromObject). 모델은 파일 경로로 로드.
 *  - writeWavFile 은 ArrayBuffer 대신 Uint8Array 를 돌려준다(expo-file-system File.write 용).
 * 나머지 UnicodeProcessor / TextToSpeech 파이프라인은 원본 그대로다.
 */
...
/**
 * 파일시스템에서 이미 읽어 파싱한 voice style JSON 객체(들)로 Style 텐서를 만든다.
 * 웹의 loadVoiceStyle 에서 fetch 부분만 걷어낸 것.
 */
export function loadVoiceStyleFromObjects(voiceStyleObjs) {
    const bsz = voiceStyleObjs.length;

    const firstStyle = voiceStyleObjs[0];
    ...
    for (let i = 0; i < bsz; i++) {
        const voiceStyle = voiceStyleObjs[i];
...
/**
 * ONNX 모델 로드. 네이티브에서는 로컬 파일 경로(file:// URI 포함)를 그대로 받는다.
 */
...
/**
 * PCM float 배열 → 16-bit WAV. Uint8Array 로 돌려준다(File.write 로 저장).
 */
