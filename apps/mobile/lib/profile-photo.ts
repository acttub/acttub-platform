/**
 * 프로필 사진 올리기의 순서(account.profile). 영상 업로드와 같은 방식이다 — 서버가 저장소
 * 주소를 주고 앱이 거기에 직접 올린 뒤 끝났다고 알린다.
 *
...
export type ProfilePhotoDependencies<Me> = {
  ...
  /** 서버가 객체를 확인하고 프로필 사진으로 바꾼다. 옛 사진 객체는 서버가 지운다. */
  complete: () => Promise<Me>;
};

export async function uploadProfilePhoto<Me>(
  pickedUri: string,
  dependencies: ProfilePhotoDependencies<Me>,
): Promise<Me> {
