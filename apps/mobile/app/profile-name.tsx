...
export function ProfileForm({ edit: isEdit }: { edit: boolean }) {
  const { profile, reloadProfile, saveProfile, closeAccountUnder14 } = useAuth();
  ...
  const complete = isProfileFormComplete(form, new Date());
  ...
  const submit = async () => {
    ...
    try {
      const now = new Date();
      await saveProfile(
        // 설정에서는 받아 둔 한 줄 소개를 그대로 실어 여섯 항목 저장이 소개를 지우지 않게 한다.
        isEdit
          ? buildProfilePayload(form, now, { bio: me?.profile?.bio ?? null })
          : buildProfilePayload(form, now),
      );
