...
test("pipeline service filters public candidates and preserves safe boundaries",()=>{const source=read("src/server/ai-pipeline-service-core.js");assert.match(source,/priority <= 3/);assert.match(source,/requireCurrentAiProcessingConsent/);assert.match(source,/repository\.claimRun/);assert.match(source,/repository\.appendPipelineTurn/);assert.match(source,/REPORT_NOT_RETRYABLE/);assert.doesNotMatch(source,/console\./);});

test("pipeline reload deletion and first-accept invariants are reachable",()=>{const route=read("src/app/api/v1/practice-sessions/[sessionId]/route.ts"),service=read("src/server/ai-pipeline-service-core.js"),migration=read("../../supabase/migrations/004_ai_pipeline_data_plane.sql");assert.match(route,/aiPipelineService\.getSession\(sessionId, auth\.userId\)/);assert.match(route,/coachSessionService\.getSession\(sessionId, auth\.userId\)/);assert.match(service,/previous\?\.status === "completed"/);assert.match(migration,/confirmation_state='accepted' and id<>p_observation_id/);assert.match(service,/completeSummaryRun/);assert.match(read("src/app/api/v1/practice-sessions/route.ts"),/aiPipelineService.createSession/);});
