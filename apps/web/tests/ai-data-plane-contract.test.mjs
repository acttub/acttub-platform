...
test("migration 009 replays under lock, expires stale reports, and removes weak overloads", () => {
  ...
  assert.match(migration009, /p_request_schema_version is null[\s\S]*p_model is null or p_prompt_version is null/);
  assert.match(migration009, /p_session_id is null or p_user_id is null or p_run_id is null or p_stage is null or p_idempotency_key is null or p_max_attempts is null/);
  assert.match(migration009, /p_request_payload_fingerprint is null/);
  ...
  assert.match(migration009, /agent_turn->'sequence' is null or jsonb_typeof\(agent_turn->'sequence'\)<>'number'/);
