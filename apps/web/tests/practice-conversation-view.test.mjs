...
test("practice.coach: 배우가 보낸 답의 수를 센다 — 영상만 올린 시작에는 배우 턴이 없다", () => {
  ...
  assert.equal(actorTurnCount({ ...conversation, messages: [conversation.messages[1]] }), 0);
});
...
test("practice.coach: 종료는 응답 상태와 대화 상태 어느 쪽으로도 알 수 있다", () => {
  ...
  assert.equal(isConversationDone({ conversation: { ...head, status: "closed" }, message: "", note: null }), true);
  assert.equal(
