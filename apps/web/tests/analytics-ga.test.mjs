...
// 박아 두지 않고 계측 판단을 브라우저에 적어 두지도 않는다(결정 I-6) — 켜도 되는지는 서버가
// 정한다: 게스트 토큰이 있고 GET /v2/consents/entry 의 privacy 행이 현재 판에 granted 일 때만
// 켠다. 새 판이 나오면 그 행이 미결정으로 돌아가 다음 조회에서 꺼진다. 그 규칙은
// tests/analytics-consent.test.mjs 의 "계측 I-6" 항목들이, 다른 탭과 동의 시트의 배선은
// tests/analytics-cross-tab.test.mjs 와 tests/consent-sheet-host.test.mjs 가 돌려 본다.
