...
test("8MB 이하는 압축을 건너뛰고 초과 파일만 압축한다", () => {
...
test("짧은 영상의 비디오 비트레이트는 1.2Mbps를 넘지 않는다", () => {
  assert.equal(computeVideoBitrate(30), MAX_VIDEO_BITRATE);
});

test("목표 크기가 상·하한 사이면 6MB 목표와 AAC 64kbps를 반영한 값을 쓴다", () => {
  const expected =
    Math.floor((0.95 * TARGET_OUTPUT_BYTES * 8) / 60) - AUDIO_BITRATE;

  assert.equal(computeVideoBitrate(60), expected);
  assert.equal(expected, 732_917);
});

test("5분 영상은 목표 크기가 아니라 300kbps 하한을 쓴다", () => {
  const target =
    Math.floor((0.95 * TARGET_OUTPUT_BYTES * 8) / 300) - AUDIO_BITRATE;

  assert.ok(target < MIN_VIDEO_BITRATE);
  assert.equal(computeVideoBitrate(300), MIN_VIDEO_BITRATE);
});

test("가로 영상은 긴 변만 720px로 줄이고 업스케일하지 않는다", () => {
  ...
  assert.deepEqual(computeOutputDimensions(720, 405), {});
  assert.deepEqual(computeOutputDimensions(640, 480), {});
});

test("세로 영상은 높이만 720px로 줄여 종횡비 계산을 mediabunny에 맡긴다", () => {
