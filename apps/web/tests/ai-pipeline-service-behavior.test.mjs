...
test("createSession double Summary reload failure maps running-only cleanup rejection safely", async () => {
  const session=baseSession();session.summary=null;session.runs=[];let reads=0,cleanupCalls=0;
  const repository={async findEligibleUpload(){return{sessionId:ids.session,storagePath:"u/s/video.mp4",requiredConsentVersionSnapshot:"v1",aiProcessingConsentVersionSnapshot:"v1"}},async createPipelineSession(){},async findPipelineSessionForOwner(){reads+=1;if(reads>=2)throw new Error("reload unavailable");return structuredClone(session)},async claimRun(input){const run={id:input.runId,stage:"summary",status:"running",safeErrorCode:null};session.runs=[run];return{owned:true,run:structuredClone(run)}},async completeSummaryRun(input){session.summary={sourceRunId:input.runId,normalizedSummary:structuredClone(input.normalizedSummary)};session.runs=[{...session.runs[0],status:"completed"}]},async failRun(){cleanupCalls+=1;throw new Error("run_not_running")}};
  const service=createAiPipelineService({repository,createAiTransport:()=>({summary:async()=>({normalizedSummary:baseSession().summary.normalizedSummary,observationCandidates:[],model:"summary-model",promptVersion:"acting-summary.prompt.v2"})}),loadAiServiceConfig:()=>({}),requireCurrentAiProcessingConsent:async()=>{},getCurrentConsentVersions:async()=>({requiredConsentVersion:"v1",aiProcessingConsentVersion:"v1"}),coachSessionService:{},createSupabaseAdminClient:()=>({storage:{from:()=>({createSignedUrl:async()=>({data:{signedUrl:"https://signed.invalid"},error:null})})}}),getAppConfig:()=>({video:{bucket:"practice-videos",signedUrlExpiresInSeconds:60}}),createSummaryNetworkError:()=>new Error("network")});
  await assert.rejects(service.createSession({sessionId:ids.session,uploadIntentId:"00000000-0000-4000-8000-000000000140",storagePath:"u/s/video.mp4",genre:"drama",situation:"scene",characterContext:"actor",subtext:null},ids.user),error=>error?.status===503&&error?.code==="SUMMARY_PERSISTENCE_FAILED");
  assert.equal(reads,3);assert.equal(cleanupCalls,1);
});

test("Report success falls back to immutable report when the post-core aggregate reload fails", async () => {
  const session=baseSession();session.interviewStatus="completed";session.completionReason="manual_stop_report_ready";
  const observationId="00000000-0000-4000-8000-000000000160",turnId="00000000-0000-4000-8000-000000000161",terminalId="00000000-0000-4000-8000-000000000162";
  session.observations=[{id:observationId,candidateId:observationId,sourceRunId:ids.summaryRun,confirmationState:"accepted",blockedForQuestioning:false,priority:1,startMs:0,endMs:1,text:"evidence",dimension:"voice",severity:"mid"}];session.reportEvidenceObservationIds=[observationId];session.reportEvidenceAnswerTurnIds=[turnId];session.transcript=[{id:turnId,sequence:0,role:"actor",kind:"answer",content:"answer",questionFocus:null,groundingStartMs:null,groundingEndMs:null,sourceObservationIds:[],reportEvidenceSelected:true}];
  const evidence={observationIds:[observationId],answerTurnIds:[turnId]};session.runs=[{id:terminalId,stage:"agent",status:"completed",responseSchemaVersion:"agent-turn.v1",model:"agent-model",promptVersion:"acting-agent.prompt.v2",completedAt:"2026-01-01T00:00:00Z",attempt:1,responsePayload:{actorTurn:null,agentTurn:{id:"00000000-0000-4000-8000-000000000163",sequence:1,role:"agent",kind:"closing",content:"done",questionFocus:null,groundingStartMs:null,groundingEndMs:null,sourceObservationIds:[],reportEvidenceSelected:false},done:true,completionReason:"manual_stop_report_ready",reportReady:true,reportEvidence:evidence}}];
  const confirmed={status:"confirmed",content:"grounded",observationEvidenceIds:[observationId],turnEvidenceIds:[turnId],timestampRange:null},review={...confirmed,turnEvidenceIds:[],timestampRange:{startMs:0,endMs:1}},empty={status:"not_confirmed",content:null,observationEvidenceIds:[],turnEvidenceIds:[],timestampRange:null};const sections={oneLineSummary:confirmed,primaryReviewPoint:review,confirmedEvidence:confirmed,actorDiscovery:{...confirmed,observationEvidenceIds:[]},groundedEncouragement:{...confirmed,turnEvidenceIds:[]},nextPracticeStep:confirmed};let immutable=null,reads=0;
  const repository={async findPipelineSessionForOwner(){reads+=1;if(reads===3)throw new Error("post-core reload unavailable");return structuredClone(session)},async claimRun(input){const run={id:input.runId,stage:"report",status:"running",idempotencyKey:input.idempotencyKey,attempt:1,maxAttempts:2,safeErrorCode:null};session.runs.push(run);return{owned:true,run:structuredClone(run)}},async completeReportRun(input){const run=session.runs.find(x=>x.id===input.runId);Object.assign(run,{status:"completed",responseSchemaVersion:"report.v1",model:input.model,promptVersion:input.promptVersion,responsePayload:{schemaVersion:"report.v1",sessionId:ids.session,runId:input.runId,model:input.model,promptVersion:input.promptVersion,sections}});immutable={sourceRunId:input.runId,schemaVersion:"report.v1",sections}},async findImmutableReport(){return structuredClone(immutable)},async failRun(){throw new Error("unexpected fail")}};
  const service=createAiPipelineService({repository,createAiTransport:()=>({report:async()=>({schemaVersion:"report.v1",sessionId:ids.session,runId:session.runs.at(-1).id,model:"report-model",promptVersion:"acting-report.prompt.v2",sections})}),loadAiServiceConfig:()=>({}),requireCurrentAiProcessingConsent:async()=>{},getCurrentConsentVersions:async()=>({}),coachSessionService:{},createSupabaseAdminClient:()=>null,getAppConfig:()=>({video:{bucket:"practice-videos"}})});
  const result=await service.retryReport(ids.session,ids.user);assert.equal(reads,3);assert.equal(result.sourceRunId,immutable.sourceRunId);assert.deepEqual(result,immutable);
});

test("tenth insufficient-evidence Agent persists a closing turn with null questionFocus", async () => {
  const session=baseSession();session.transcript=Array.from({length:9},(_,i)=>({id:`00000000-0000-4000-8000-${String(200+i).padStart(12,"0")}`,sequence:i,role:"actor",kind:"unknown",content:"unknown",questionFocus:null,groundingStartMs:null,groundingEndMs:null,sourceObservationIds:[],reportEvidenceSelected:false}));let appended=null;
  const repository={async findPipelineSessionForOwner(){return structuredClone(session)},async claimRun(input){const run={id:input.runId,stage:"agent",status:"running",safeErrorCode:null};session.runs=[run];return{owned:true,run:structuredClone(run)}},async appendPipelineTurn(input){appended=structuredClone(input);session.transcript.push(input.actorTurn,input.agentTurn);session.substantiveAnswerCount=1;session.interviewStatus="completed_without_report";session.completionReason="insufficient_interview_evidence";session.runs=[{...session.runs[0],status:"completed",responseSchemaVersion:"agent-turn.v1",model:input.model,promptVersion:input.promptVersion,responsePayload:structuredClone(input.responsePayload)}]},async failRun(){throw new Error("unexpected fail")}};
  const service=createAiPipelineService({repository,createAiTransport:()=>({agent:async()=>({action:"close",utterance:"done",evidence:{observationIds:[]},reportEvidence:{observationIds:[],answerTurnIds:[]},done:true,completionReason:"insufficient_interview_evidence",reportReady:false,model:"agent-model",promptVersion:"acting-agent.prompt.v2"})}),loadAiServiceConfig:()=>({}),requireCurrentAiProcessingConsent:async()=>{},getCurrentConsentVersions:async()=>({}),coachSessionService:{},createSupabaseAdminClient:()=>null,getAppConfig:()=>({video:{bucket:"practice-videos"}})});
  const result=await service.addTurn(ids.session,ids.user,{answer:"answer",requestId:ids.request,expectedSubstantiveAnswerCount:0,expectedTotalConversationCount:9});assert.equal(result.done,true);assert.equal(result.reportReady,false);assert.equal(appended.agentTurn.kind,"closing");assert.equal(appended.agentTurn.questionFocus,null);assert.equal(appended.completionStatus,"completed_without_report");
});
