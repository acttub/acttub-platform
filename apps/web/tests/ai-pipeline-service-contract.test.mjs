...
test("terminal agent turns are committed through the append pipeline transaction", () => {
  ...
  assert.match(callAgent, /completionStatus:invoked.done\|\|String\(invoked.transportResponse\?\.action\)==="pause"\?\(invoked.reportReady\?"completed":String\(invoked.transportResponse\?\.action\)==="pause"\?"paused":"completed_without_report"\):null/);
