...
- **서버에서 하나를 읽어 화면에 그릴 때는 `lib/react/use-resource.ts`의 `useResource`를 씁니다.** 취소·오류 문구·로딩 종료를 UI에서 다시 만들지 않습니다. 재조회 조건은 키 하나(원시값, `null`이면 게이트)로 주고, `load`는 그 키를 인자로 받습니다 — 그래서 인라인 화살표로 적어도 매 렌더 다시 묻지 않습니다.
- 예외는 **답을 그 뒤 사용자 행동이 계속 바꾸는 화면**입니다(목록 누적·폼 초기값 시딩·칸 편집). 그런 자리는 훅이 데이터를 들면 같은 답을 state가 한 벌 더 들게 되므로 조회를 그대로 둡니다. 어느 자리가 왜 그런지는 각 이펙트 위에 적혀 있습니다(SOMA-411).
