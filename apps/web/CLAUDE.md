...
- **서버에서 하나를 읽어 화면에 그릴 때는 `lib/react/use-resource.ts`의 `useResource`를 씁니다.** 취소·오류 문구·로딩 종료를 UI에서 다시 만들지 않습니다. 재조회 조건은 키 하나(원시값, `null`이면 게이트)로 줍니다 — 이펙트가 그 키만 보므로 `load`를 인라인 화살표로 적어도 매 렌더 다시 묻지 않습니다. `load`가 그 키를 **인자로** 받는 것은 별개 이유입니다: 키를 클로저로 가두지 않아야 어느 렌더의 `load`든 같은 일을 합니다.
- 예외는 **답을 그 뒤 사용자 행동이 계속 바꾸는 화면**입니다 — 목록 누적, 폼 초기값 시딩, 칸 편집, 그리고 같은 답을 세우는 길이 둘 이상인 경우(첫 진입 + 수동 재조회). 그런 자리는 훅이 데이터를 들면 같은 답을 state가 한 벌 더 들게 되므로 조회를 그대로 둡니다. `SOMA-411`이 판정한 자리들은 각 이펙트 위에 이유가 적혀 있습니다.
- 오류를 화면에 보일 한 줄로 바꾸는 것은 `api/v2/errors.ts`의 `errorMessage` 하나입니다. `cause instanceof Error ? cause.message : "…"`로 새로 적지 않습니다 — 빈 message를 방어하지 않아 아무 말 없는 오류 자리가 뜹니다.
