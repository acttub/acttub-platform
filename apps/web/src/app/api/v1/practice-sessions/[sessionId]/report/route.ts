...
export async function POST(request: Request, context: Context) { try { const auth = await requireApiTermsAccepted(); const { sessionId } = await context.params; return jsonResponse(await actingCoachService.createReport(sessionId, await request.json(), auth.userId), { status: 201 }); } catch (error) { return handleApiError(error); } }
