...
async function runAnalysisClaim(claim: Claim, userId: string) {
  ...
  try { const response = await actingApiClient.summarize({ situation: String(source.formattedSituation), character: String(source.characterContext), subtext: String(source.subtext), video: video.body, fileName: String(source.fileName ?? "take.mp4"), mimeType: String(source.mimeType) }); const summary = requireRecord(await json(response)); await repository.completeAnalysis({ userId, sessionId: claim.sessionId, operationId: claim.operationId, leaseToken: claim.leaseToken, sceneSummaryId: crypto.randomUUID(), summaryPayload: summary }); return ownedSession(userId, String(claim.sessionId)); } catch (error) { if (error instanceof ActingServiceError) await repository.failAnalysis({ userId, sessionId: claim.sessionId, operationId: claim.operationId, leaseToken: claim.leaseToken, failureClass: error.status === 429 || error.code === "acting_api_auth_failed" ? "definitive" : "ambiguous", safeErrorCode: error.code }); throw error; }
}
