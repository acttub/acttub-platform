...
type ActingRepository = {
  createAnalysisClaim(input: Record<string, unknown>): Promise<Claim>;
  claimAnalysisRetry(input: Record<string, unknown>): Promise<Claim>;
  completeAnalysis(input: Record<string, unknown>): Promise<void>;
  failAnalysis(input: Record<string, unknown>): Promise<void>;
  claimCoachStart(input: Record<string, unknown>): Promise<Claim>;
  claimCoachReply(input: Record<string, unknown>): Promise<Claim>;
  completeCoachTurn(input: Record<string, unknown>): Promise<void>;
  failCoachOperation(input: Record<string, unknown>): Promise<void>;
  expireCoachRun(input: Record<string, unknown>): Promise<void>;
  claimReport(input: Record<string, unknown>): Promise<Claim>;
  completeReport(input: Record<string, unknown>): Promise<void>;
  failReport(input: Record<string, unknown>): Promise<void>;
  getOwnedSession(userId: string, sessionId: string): Promise<ActingCoachSessionDto | null>;
  getOwnedReport(userId: string, sessionId: string): Promise<ActingReportDto | null>;
};
const repository = supabaseCoachSessionRepository as unknown as ActingRepository;
...
const ownedSession = async (userId: string, sessionId: string) => { const result = await repository.getOwnedSession(userId, sessionId); if (!result) throw new ActingServiceError(404, "session_not_found", "Session was not found."); return result; };
...
export const actingCoachService = {
  async createSession(payload: unknown, userId: string) {
    ...
    const claim = await repository.createAnalysisClaim({ ...operationInput(userId, input.requestId, ["create", uploadIntentId, medium, genre, situation, characterContext, subtext]), uploadIntentId, sessionId: crypto.randomUUID(), takeId: crypto.randomUUID(), medium, genre, situation, characterContext, subtext, leaseSeconds: 780 });
    ...
    try { const response = await actingApiClient.summarize({ situation: `[매체: ${medium}] [장르: ${genre}] ${situation}`, character: characterContext, subtext, video: video.body, fileName: String(source.fileName ?? "take.mp4"), mimeType: String(source.mimeType) }); const summary = requireRecord(await json(response)); await repository.completeAnalysis({ userId, sessionId: claim.sessionId, operationId: claim.operationId, leaseToken: claim.leaseToken, sceneSummaryId: crypto.randomUUID(), summaryPayload: summary }); return ownedSession(userId, String(claim.sessionId)); } catch (error) { if (error instanceof ActingServiceError) await repository.failAnalysis({ userId, sessionId: claim.sessionId, operationId: claim.operationId, leaseToken: claim.leaseToken, failureClass: error.status === 429 || error.code === "acting_api_auth_failed" ? "definitive" : "ambiguous", safeErrorCode: error.code }); throw error; }
  },
  async retryAnalysis(sessionId: string, payload: unknown, userId: string) { const input = payload as RetryAnalysisRequest; if (input.operation !== "retry") throw new ActingServiceError(400, "validation_error", "operation must be retry."); const claim = await repository.claimAnalysisRetry({ ...operationInput(userId, input.requestId, ["analysis_retry", uuid(sessionId, "sessionId")]), sessionId, leaseSeconds: 780 }); if (claim.kind !== "claimed") return replay(claim); throw new ActingServiceError(409, "analysis_outcome_unknown", "Analysis retry requires the persisted transfer source.", { action: "create_new_session" }); },
  async turn(sessionId: string, payload: unknown, userId: string) {
    ...
    if (isStart) { const claim = await repository.claimCoachStart({ ...operationInput(userId, input.requestId, [input.operation, sid]), sessionId: sid, runId: crypto.randomUUID(), leaseSeconds: 120, restart: input.operation === "restart" }); if (claim.kind !== "claimed") return replay(claim); const response = requireCoachResponse(await json(await actingApiClient.start({ summary: claim.summaryPayload, subtext: claim.coachContext }))); await repository.completeCoachTurn({ userId, sessionId: sid, runId: claim.runId, operationId: claim.operationId, leaseToken: claim.leaseToken, actingSessionId: response.session_id, aiTurnId: crypto.randomUUID(), question: response.question, action: response.action, focusTimestamp: response.focus_timestamp, done: response.done, closeReason: response.close_reason, responsePayload: response }); return ownedSession(userId, sid); }
    const runId = uuid(input.runId, "runId"); const text = input.operation === "reply" ? normalizeContext(input.text, "text") : undefined; const actorTurnId = input.operation === "retry_reply" ? uuid(input.actorTurnId, "actorTurnId") : crypto.randomUUID(); const claim = await repository.claimCoachReply({ ...operationInput(userId, input.requestId, [input.operation, sid, runId, ...(text ? [text] : [actorTurnId])]), sessionId: sid, runId, actorTurnId, actorText: text, retryActorTurnId: input.operation === "retry_reply" ? actorTurnId : null, leaseSeconds: 120 }); if (claim.kind !== "claimed") return replay(claim); const response = requireCoachResponse(await json(await actingApiClient.reply({ session_id: claim.privateActingSessionId, text: claim.source?.actorText ?? text })), claim.privateActingSessionId); await repository.completeCoachTurn({ userId, sessionId: sid, runId, operationId: claim.operationId, leaseToken: claim.leaseToken, actingSessionId: response.session_id, aiTurnId: crypto.randomUUID(), question: response.question, action: response.action, focusTimestamp: response.focus_timestamp, done: response.done, closeReason: response.close_reason, responsePayload: response }); return ownedSession(userId, sid);
  },
  ...
  async createReport(sessionId: string, payload: unknown, userId: string) { const requestId = (payload as { requestId?: unknown }).requestId; const sid = uuid(sessionId, "sessionId"); const claim = await repository.claimReport({ ...operationInput(userId, String(requestId), ["report", sid]), sessionId: sid, leaseSeconds: 120 }); if (claim.kind !== "claimed") return replay(claim); const response = requireReportResponse(await json(await actingApiClient.report(claim.reportPayload)), userId); await repository.completeReport({ userId, sessionId: sid, operationId: claim.operationId, leaseToken: claim.leaseToken, reportId: crypto.randomUUID(), reportPayload: response.report, reportCount: response.report_count, responsePayload: response }); return this.getReport(sid, userId); },
};
