...
/**
 * 단계 페이지가 공통으로 하는 일 — 들고 있는 초안·대본·설정·결과를 읽고, 앞 단계가 비어 있으면
...
export function useReadingStep(step: Step): ReadingStep {
  ...
  const to = hydrated
    ? redirectFor(step, { hasDraft: !!draft, hasScript: !!script, hasSetup: !!setup, hasStats: !!stats, desktop })
    : null;
  ...
  return { draft, script, setup, stats, ready: hydrated && !to, desktop };
}
