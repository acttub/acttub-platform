    ...
    getAuthSession()
      .then((session) => {
        ...
        if (!session.authenticated) {
          window.location.href = "/auth/login?next=/home";
          return;
        }
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-2xl flex-col justify-center px-6 py-12">
      ...
      <button
        type="button"
        disabled={
          state.kind !== "ready" || submitting || !serviceConsent || !aiProcessingConsent
        }
...
function ConsentCheckbox({
  ...
  return (
    <label className="flex cursor-pointer items-start gap-3 text-sm font-semibold leading-6 text-[#4e5968]">
      ...
      />
      <span>
        {children} {required ? <strong className="text-[#3182f6]">(필수)</strong> : null}
      </span>
    </label>
  );
}
