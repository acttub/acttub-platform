...
import { optionalNoteLength } from "@/lib/optional-note.js";
    ...
    <main className="mx-auto w-full max-w-5xl px-5 py-8 md:py-12">
      ...
      {session ? (
        <div className="space-y-8">
          {session.interviewStatus==="completed"||session.interviewStatus==="completed_without_report"?<section className="rounded-2xl bg-white p-5"><label htmlFor="optional-note" className="font-semibold">내 문장 남기기 (선택)</label><textarea id="optional-note" value={optionalNote} onChange={event=>{setOptionalNote(event.target.value);setNoteStatus("");}} disabled={savingNote} className="mt-3 min-h-28 w-full rounded-xl border border-slate-300 p-3" aria-describedby="optional-note-count optional-note-status"/><p id="optional-note-count" className="mt-1 text-right text-xs text-slate-500">{optionalNoteLength(optionalNote)}/1000</p><p id="optional-note-status" aria-live="polite" className="mt-2 min-h-5 text-sm text-emerald-700">{noteStatus}</p><div className="mt-3 flex gap-2"><button type="button" onClick={()=>void persistOptionalNote(optionalNote)} disabled={savingNote||optionalNoteLength(optionalNote)>1000} className="rounded-full bg-slate-900 px-4 py-2 text-sm text-white disabled:opacity-50">{savingNote?"저장 중":"저장"}</button><button type="button" onClick={()=>void persistOptionalNote(null)} disabled={savingNote||(!session.optionalNote&&!optionalNote)} className="rounded-full border border-slate-300 px-4 py-2 text-sm disabled:opacity-50">지우기</button></div></section>:null}
