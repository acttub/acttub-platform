...
public final class ObservationPrompt {
    ...
    public static final String SYSTEM = """
            ...
            영상 전체를 처음부터 끝까지 시간 순서로 이어서 적는다. 구간별 observations 는 조각조각
            떨어져 있어 축이 서로 어떻게 겹치는지가 남지 않는다 — 말이 빨라지는 것과 시선이 흔들리는
            것이 같은 시점인지, 어디서 흐름이 한 번 바뀌는지를 여기서 잡는다. 시각을 함께 적는다.
            왜 그런지, 잘했는지 못했는지는 쓰지 않는다.
