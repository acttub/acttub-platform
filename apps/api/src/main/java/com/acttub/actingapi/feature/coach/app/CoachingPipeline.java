...
/** Luna selects the next conversational action; Java dispatches one generation prompt. */
final class CoachingPipeline {
    ...
    private static final JsonNode ROUTE_SCHEMA = StructuredJson.parse("""
            ...
            "enum":["advance","scaffold","repair","respond"]}},"required":["route"]}
            """);
    ...
    private String call(CoachSessionSnapshot session, LlmStep step, String prompt, JsonNode input,
        ...
        var metadata = LlmCall.metadata("contract", "dialogue_actions_v2", "route", route == null ? "" : route.id);
        try {
