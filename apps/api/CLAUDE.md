...
📌 **SOMA-460으로 운영 DB 접근을 JPA에 일원화했습니다.** Schema Entity는 26개 테이블을 모두 매핑하며 자기 feature의 Adapter와 영속 내부 repository만 사용할 수 있습니다. app·domain과 다른 feature는 Schema Entity나 Spring Data interface를 보면 안 됩니다. 운영 직접 JDBC 접근은 없고, native DML `RETURNING`은 [CONTRACT.md](CONTRACT.md) §5-2·§5-8의 data-modifying CTE + alias `Tuple` 패턴을 씁니다.
