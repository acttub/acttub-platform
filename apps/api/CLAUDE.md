...
- ⚠ **보호 요청의 필수 동의 게이트는 위 보류 목록을 재사용하지 않습니다.** `RequiredConsentGate`가 최신 필수 문서만 보고 허용·미결정·차단을 가릅니다. `X-Acttub-Consent-Entry: 1`을 보내는 새 클라이언트에는 차단을 `403 consent_blocked`로, 헤더가 없는 구형 클라이언트에는 종전대로 `403 consent_required`로 답합니다. 동의 조회·기록 라우트는 이 게이트 표에 넣지 않습니다.
