...
class CoachValidatorTest {
    ...
    void scansLiteralAfterNfkcNormalizationAndKeepsCanonicalTerm() {
        assertThat(CoachValidator.scanForbidden("ＡＮＡＬＹＳＩＳ 결과입니다"))
    ...
    void reportsFailuresInPythonValidationOrderWithExactMessages() {
        ...
        CoachValidation result = CoachValidator.validateTurn(message, true);
    ...
    void coachDisablesSentenceLimit() {
        CoachValidation result = CoachValidator.validateTurn("가".repeat(171), false);
    ...
    void detectsEachCanonicalTimecodePattern(String text) {
        assertThat(CoachValidator.hasTimecode(text)).isTrue();
    }

    @ParameterizedTest
    @MethodSource("forbiddenRegexes")
    void detectsEachCanonicalForbiddenRegex(String text, String label) {
        assertThat(CoachValidator.scanForbidden(text)).containsExactly(label);
    }
