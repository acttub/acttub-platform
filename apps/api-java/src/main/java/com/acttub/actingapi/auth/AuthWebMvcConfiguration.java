 ...
 * 알면 {@code auth → web}(예외 타입 참조)과 맞물려 순환이 된다 (ADR-016). 순서 값은
 * {@link InterceptorOrder}가 소유한다.
 */
@Configuration(proxyBeanMethods = false)
class AuthWebMvcConfiguration implements WebMvcConfigurer {
    private final ConsentGateInterceptor consentGate;

    AuthWebMvcConfiguration(ConsentGateInterceptor consentGate) {
