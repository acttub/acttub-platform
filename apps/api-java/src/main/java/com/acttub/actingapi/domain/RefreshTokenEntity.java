...
public class RefreshTokenEntity extends AppGeneratedUuidEntity {
    ...
    public RefreshTokenEntity(UUID id,UUID userId,String tokenHash,String deviceInfo,Instant expiresAt){super(id);this.userId=userId;this.tokenHash=tokenHash;this.deviceInfo=deviceInfo;this.expiresAt=expiresAt;}
    public UUID getUserId(){return userId;} public UUID getReplacedById(){return replacedById;} public String getTokenHash(){return tokenHash;} public Instant getExpiresAt(){return expiresAt;} public Instant getRevokedAt(){return revokedAt;}
    public void replaceWith(UUID replacement,Instant now){replacedById=replacement;revokedAt=now;} public void revoke(Instant now){revokedAt=now;}
}
