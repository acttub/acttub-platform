...
/** {@link CoachOperationLedger} 를 공용 External Operation 원장 위에 얹는다. */
@Component
class ExternalCoachOperationLedger implements CoachOperationLedger {

    private final SyncOperationService sync;

    ExternalCoachOperationLedger(SyncOperationService sync) {
        this.sync = sync;
    }
