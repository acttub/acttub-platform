...
- 🔥 **포트가 없음을 알리는 방법은 하나가 아닙니다.** `practice`는 `null`·`boolean`으로 알리지만 `community`는 **예외를 `app`에 선언하고 어댑터가 던집니다.** 갈리는 기준은 *한 연산 안에서 갈릴 것이 몇 가지냐*입니다 — `updatePost`는 "없다"와 "내 것이 아니다" 둘이고, 그 판정이 같은 트랜잭션·같은 잠금 안에서 일어나야 해서 `null` 하나로는 못 가릅니다. 예외가 `app`에 살면 방향이 `adapter → app`이라 층 규칙에 맞고, 제공자와 소비자가 같은 도메인이라 시그니처에 남의 패키지 이름이 보이지도 않습니다.
