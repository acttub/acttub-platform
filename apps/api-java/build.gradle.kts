...
tasks.named<org.gradle.language.jvm.tasks.ProcessResources>("processResources") {
    from("../api/acting-api/consent_docs") {
        // 파일 이름을 하나씩 적지 않는다. 방침 버전이 오르면(privacy_v2 → v3, SOMA-326)
        // manifest 는 새 파일을 가리키는데 여기 목록은 그대로라 **부팅이 FileNotFoundException 으로
        // 죽는다** — 실제로 그렇게 깨졌다. 과거 버전 문서도 함께 넣는다(빈 DB 재구축 경로가 쓴다).
        include("manifest.json", "*.md")
        exclude("README.md")
        into("consent-docs")
    }
