...
#
# 마이그레이션은 두 환경 모두 배포 중에 자동으로 돈다(ssm-deploy.sh 의 migrate 모드).
# 그래도 head가 갈리거나 부분 적용되면 코드는 새것·DB는 구것인 상태가 남는데,
# 겉으로는 배포가 성공으로 보인다. 2026-08-01에 그 상태로 나가 커뮤니티 API가
# 며칠간 500이었다(community_posts.anonymous 없음). 그래서 배포 직후 대조한다.
#
