...
# 다시 구워 위 재생성이 일어난다.
#   0  있다
#   1  없다 — 레지스트리가 manifest unknown(로그인 상태) 또는 denied(익명·패키지 없음)로 답했다
#   2  판정 불가 — 네트워크·레지스트리 오류. 부르는 쪽이 실패로 둔다
# 실측(2026-09-03, docker 29): 없는 태그·아직 없는 패키지 모두 로그인 뒤엔 "manifest unknown",
# 익명이면 "denied". 호스트를 못 찾으면 "failed to configure transport … no such host".
set -euo pipefail
...
case "$out" in
  *"manifest unknown"*|*denied*|*"not found"*)
    echo "없다: $IMAGE ($out)"
    exit 1 ;;
  *)
