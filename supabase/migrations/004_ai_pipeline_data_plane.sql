...
returns jsonb language plpgsql security definer set search_path=public as $$ declare c jsonb; a jsonb; take uuid; existing_run uuid; idx integer; begin
 ...
 select id into take from public.practice_takes where session_id=p_session_id and user_id=p_user_id order by created_at limit 1; if take is null then raise exception 'take_not_found'; end if;
 ...
 for c,idx in select value,ordinality::integer from jsonb_array_elements(p_candidates) with ordinality loop a:=p_summary->'anomalies'->(idx-1); if (select array_agg(k order by k) from jsonb_object_keys(c) k)<>array['dimension','endMs','id','priority','severity','startMs','text'] or (c->>'priority')::integer<>idx or c->>'text'<>a->>'what' or c->>'dimension'<>a->>'dimension' or c->'severity' is distinct from a->'severity' or (c->>'startMs')::integer<0 or (c->>'endMs')::integer<(c->>'startMs')::integer then raise exception 'invalid_summary_candidate'; end if;
