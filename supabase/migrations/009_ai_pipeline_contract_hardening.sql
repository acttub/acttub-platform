...
begin
 ...
 if jsonb_typeof(agent) is distinct from 'object' or (select array_agg(k order by k) from jsonb_object_keys(agent) k) is distinct from array['content','groundingEndMs','groundingStartMs','id','kind','questionFocus','reportEvidenceSelected','role','sequence','sourceObservationIds'] or jsonb_typeof(agent->'id') is distinct from 'string' or agent->>'id' !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' or jsonb_typeof(agent->'sequence') is distinct from 'number' or (agent->>'sequence') !~ '^(0|[1-9][0-9]*)$' or agent->>'role' is distinct from 'agent' or jsonb_typeof(agent->'content') is distinct from 'string' or length(trim(agent->>'content'))=0 or agent->'groundingStartMs' is distinct from 'null'::jsonb or agent->'groundingEndMs' is distinct from 'null'::jsonb or jsonb_typeof(agent->'sourceObservationIds') is distinct from 'array' or jsonb_typeof(agent->'reportEvidenceSelected') is distinct from 'boolean' or (agent->>'reportEvidenceSelected')::boolean then raise exception 'invalid_agent_turn'; end if;
 if actor is not null and actor<>'null'::jsonb and (jsonb_typeof(actor) is distinct from 'object' or (select array_agg(k order by k) from jsonb_object_keys(actor) k) is distinct from array['content','groundingEndMs','groundingStartMs','id','kind','questionFocus','reportEvidenceSelected','role','sequence','sourceObservationIds'] or jsonb_typeof(actor->'id') is distinct from 'string' or actor->>'id' !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' or jsonb_typeof(actor->'sequence') is distinct from 'number' or (actor->>'sequence') !~ '^(0|[1-9][0-9]*)$' or actor->>'role' is distinct from 'actor' or actor->>'kind' not in ('answer','unknown') or jsonb_typeof(actor->'content') is distinct from 'string' or length(trim(actor->>'content'))=0 or actor->'groundingStartMs' is distinct from 'null'::jsonb or actor->'groundingEndMs' is distinct from 'null'::jsonb or actor->'questionFocus' is distinct from 'null'::jsonb or jsonb_typeof(actor->'sourceObservationIds') is distinct from 'array' or jsonb_typeof(actor->'reportEvidenceSelected') is distinct from 'boolean') then raise exception 'invalid_actor_turn'; end if;
 if jsonb_typeof(p_payload->'reportEvidence') is distinct from 'object' or (select array_agg(k order by k) from jsonb_object_keys(p_payload->'reportEvidence') k) is distinct from array['answerTurnIds','observationIds'] or jsonb_typeof(p_payload->'reportEvidence'->'observationIds') is distinct from 'array' or jsonb_typeof(p_payload->'reportEvidence'->'answerTurnIds') is distinct from 'array' then raise exception 'invalid_report_evidence'; end if;
