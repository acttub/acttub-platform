...
returns jsonb language plpgsql security definer set search_path=public as $$ declare reason text:=p_payload->>'completionReason'; stat text:=p_payload->>'status'; obs uuid[]:=array(select jsonb_array_elements_text(p_payload->'observationIds'))::uuid[]; turns uuid[]:=array(select jsonb_array_elements_text(p_payload->'answerTurnIds'))::uuid[]; n integer; conversation_count integer; next_sequence integer; last_two_kinds text[]; agent_run_id text:=p_payload->>'agentRunId'; agent_response jsonb:=p_payload->'agentResponse'; agent_turn jsonb:=p_payload->'agentResponse'->'agentTurn'; begin
 ...
 if agent_run_id is not null then
  ...
  if agent_response is null or jsonb_typeof(agent_response)<>'object' or (select array_agg(k order by k) from jsonb_object_keys(agent_response) k)<>array['actorTurn','agentTurn','completionReason','done','reportEvidence','reportReady'] or agent_response->'actorTurn'<>'null'::jsonb or jsonb_typeof(agent_turn)<>'object' or (select array_agg(k order by k) from jsonb_object_keys(agent_turn) k)<>array['content','groundingEndMs','groundingStartMs','id','kind','questionFocus','reportEvidenceSelected','role','sequence','sourceObservationIds'] or agent_turn->>'id' !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' or (agent_turn->>'sequence')::integer<>next_sequence or agent_turn->>'role'<>'agent' or length(trim(coalesce(agent_turn->>'content','')))=0 or agent_turn->'groundingStartMs'<>'null'::jsonb or agent_turn->'groundingEndMs'<>'null'::jsonb or agent_turn->'sourceObservationIds'<>'[]'::jsonb or agent_turn->>'reportEvidenceSelected'<>'false' or jsonb_typeof(agent_response->'done')<>'boolean' or (agent_response->>'done')::boolean is distinct from (stat<>'paused') or agent_response->>'completionReason' is distinct from reason or jsonb_typeof(agent_response->'reportReady')<>'boolean' or (agent_response->>'reportReady')::boolean is distinct from (stat='completed') or agent_response->'reportEvidence' is distinct from jsonb_build_object('observationIds',to_jsonb(obs),'answerTurnIds',to_jsonb(turns)) or (stat='paused' and agent_turn->>'kind'<>'question') or (stat<>'paused' and agent_turn->>'kind'<>'closing') then raise exception 'invalid_agent_completion_payload'; end if;
