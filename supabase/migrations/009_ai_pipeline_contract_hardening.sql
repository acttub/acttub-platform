...
returns jsonb language plpgsql security definer set search_path=public as $$ declare reason text:=p_payload->>'completionReason'; stat text:=p_payload->>'status'; obs uuid[]:=array(select jsonb_array_elements_text(p_payload->'observationIds'))::uuid[]; turns uuid[]:=array(select jsonb_array_elements_text(p_payload->'answerTurnIds'))::uuid[]; n integer; conversation_count integer; last_two_kinds text[]; agent_run_id text:=p_payload->>'agentRunId'; begin
 ...
 if agent_run_id is not null and length(trim(agent_run_id))>0 then
  if p_payload->'agentResponse' is null or jsonb_typeof(p_payload->'agentResponse')<>'object' or (select array_agg(k order by k) from jsonb_object_keys(p_payload->'agentResponse') k)<>array['actorTurn','agentTurn','completionReason','done','reportEvidence','reportReady'] or (p_payload->'agentResponse'->'actorTurn'<>'null'::jsonb and jsonb_typeof(p_payload->'agentResponse'->'actorTurn')<>'object') or jsonb_typeof(p_payload->'agentResponse'->'agentTurn')<>'object' or p_payload->'agentResponse'->>'done'<>'true' or p_payload->'agentResponse'->>'completionReason' is distinct from reason or (p_payload->'agentResponse'->>'reportReady')::boolean is distinct from (stat='completed') or p_payload->'agentResponse'->'reportEvidence' is distinct from jsonb_build_object('observationIds',to_jsonb(obs),'answerTurnIds',to_jsonb(turns)) then raise exception 'invalid_agent_completion_payload'; end if;
  perform 1 from public.ai_runs where id=agent_run_id::uuid and session_id=p_session_id and user_id=p_user_id and stage='agent' and status='running' and request_schema_version='agent-turn.v1' and model=p_payload->>'model' and length(trim(coalesce(model,'')))>0 and prompt_version='acting-agent.prompt.v2' for update; if not found then raise exception 'agent_run_not_running'; end if;
  update public.ai_runs set status='completed',response_schema_version='agent-turn.v1',response_payload=p_payload->'agentResponse',completed_at=now(),updated_at=now() where id=agent_run_id::uuid;
 end if;
