...
**사이클별로 단독 구현을 택할 수 있다.** 이중 구현은 비용이 두 배이므로, 완료 기준이 충분히 촘촘해 Phase 5~6 리뷰만으로 오류가 드러난다면 단독으로 간다. **M2는 사용자 결정으로 Codex 단독**이다(2026-08-08) — 대신 Phase 2 비판에서 나온 완료 기준 강화를 전부 반영했다.
