...
5. **검증** (6장). 브라우저 확인은 사용자가 한다. 배포 스크립트의 성공 판정과 `/health`는
   S3를 보지 않으므로(`app.py:194-206`, `ssm-deploy.sh:63-68`) **자동 판정을 믿지 말고 직접
   확인한다.**
...
- dev 버킷 객체 2개 / 운영 103개. 이 중 만료 intent 잔여물이 얼마인지는 5장의 별도 티켓에서
  다룬다.
- **기각한 지적**: "기본 체인이라 `~/.aws/credentials`가 우선해 경계가 안 생긴다"(Codex
  HOLES 1) — 두 EC2에 공유 자격증명 파일이 없고, 있더라도 D3의 `method` 로그와 4장의
  preflight에서 즉시 드러난다.
