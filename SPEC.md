...
🔁 **검사기는 은퇴했다(`SOMA-403` 5단계).** `spec/check-refs.py`는 모든 SPEC 문서의 참조를 훑어 파일과 심볼이 실재하는지 검사했고 `ci.yml`이 매 PR 마다 돌렸다. **검사 대상 304건이 전부 파이썬 심볼이었다** — 정규식이 `.py` 만 잡는다. 파이썬 트리가 사라지면서 검사할 것이 하나도 남지 않아, 검사기와 CI 스텝을 함께 지웠다.

폐기된 도구를 면제하는 길(4단계가 하네스에 쓴 방식)도 있었지만 **304건 전부가 면제 대상이라 "오류 0"이 아무것도 뜻하지 않게 된다.** 초록으로 끝나는 길이 둘이면 약한 쪽이 쓰인다.

아래 참조 규약 자체는 남은 이관 기간 동안 유효하다 — 다만 지키는 것은 검사기가 아니라 사람이다. M0 문서(`spec/M0-spike.md`·`spec/M0-findings.md`)는 **그 시점의 기록**이므로 라인 참조를 그대로 둔다.
